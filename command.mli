...
(** [parse_user input] parses a user's input into a [command].
    The first word of [input] becomes the verb. The rest of the words,
    if any, become the object phrase.

    Requires: [input] contains only alphanumeric (A-Z, a-z, 0-9) and space 
    characters (only ASCII character code 32; not tabs or newlines, etc.).

    Raises: [Malformed] if the command is malformed. A command
    is malformed if [input] is the empty string or contains only spaces,
    or if the verb is not a command type,
    or if the verb is "send" and first word in [input] is not a valid user,
    or if the verb is "view" and there is an empty or invalid object phrase,
    or if the verb is "reset" and there is an empty or invalid object phrase,
...
(** [parse_admin input] parses an admin's input into an [acommand].
    The first word of [input] becomes the verb. The rest of the words,
    if any, become the object phrase.
    ...
    or if the verb is "hist" and the object phrase is empty or not a 
    valid Survey.qid,
    or if the verb is "dist" and the object phrase is empty or not a 
    pair of valid Client usernames,
